...
static unsigned int g_VboHandle = 0, g_ElementsHandle = 0;
	...
	ImTextureID CreateAndBindTexture(uint8_t* pixels, int32_t width,
		int32_t height, int32_t comp)
	{
		// back up opengl state
		GLint last_texture;
...
bool ImGui_ImplGlfwGL3_BindImage(const std::string& path,
	ImageTexture* out)
{
	if (!out)
	{
	...
	// parse image
	pixels = stbi_load(path.c_str(), &width, &height, &comp, 0);
	if (pixels == nullptr)
	{
		return false;
	}
	...
	// load the image data into gpu
	auto texture_id = CreateAndBindTexture(pixels, width, height, comp);

	// free parsed image
	stbi_image_free(pixels);

	out->texture_id = texture_id;
	out->size = ImVec2(static_cast<float>(width),
...
bool ImGui_ImplGlfwGL3_BindBMP(uint8_t* data, size_t size, void** out_texture, int width, int height) {
	if (!out_texture)
		return false;
	...
	glPixelStorei(GL_UNPACK_ROW_LENGTH, dim.x);  
	glPixelStorei(GL_UNPACK_IMAGE_HEIGHT, dim.y); 
	glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0);
	glPixelStorei(GL_UNPACK_SKIP_ROWS, 0);
	glPixelStorei(GL_UNPACK_SKIP_IMAGES, 0);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 4); // For TrueColor DIBs the alignment is 4 */
    ...
    *out_texture = reinterpret_cast<void*>(static_cast<intptr_t>(texture_name));
	return true;
}

bool ImGui_ImplGlfwGL3_BindImage(uint8_t* data,
	size_t length,
	ImageTexture* out)
{
	if (!out)
	...
	// parse image
	pixels = stbi_load_from_memory(data, static_cast<int>(length),
		&width, &height,
		&comp, 0);
	if (pixels == nullptr)
	{
